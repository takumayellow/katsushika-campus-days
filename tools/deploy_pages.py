...
# data の上限は 64 MiB。実測より 21%（11.1 MiB）上。data は gzip を解いた中身が, 遊んでいる間ずっと
#   ブラウザのメモリ（Unity のヒープの外にある仮想のファイルシステム）に載る（実測の比 1.31 なら 64 MiB で
#   約 84 MiB）ので, タブのメモリの予算（docs/WEBGL_BUDGET.md）に響く。
