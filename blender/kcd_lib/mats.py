...
PALETTE = {
    ...
    "brick_red":       ((0.195, 0.107, 0.080), 0.85, 0.0, 1.0),  # #7A5C50
    ...
    # 葛飾コミュニティハウス（kcd_route/dorm.py）。実物の写真と航空写真から拾った色
    "dorm_white":      ((0.807, 0.855, 0.831), 0.70, 0.0, 1.0),  # #E8EEEB スラブの小口・裏面・塔屋
    "dorm_cream":      ((0.776, 0.701, 0.565), 0.80, 0.0, 1.0),  # #E4DAC6 バルコニー奥の外壁
    "glass_frosted":   ((0.552, 0.672, 0.701), 0.20, 0.0, 0.72),  # #C4D6DA バルコニーの型板ガラス（薄い青緑）
    "tile_mauve":      ((0.102, 0.051, 0.040), 0.75, 0.0, 1.0),  # #5A4038 角の階段室の小口タイル（チョコレート色）
    "tile_charcoal":   ((0.209, 0.195, 0.216), 0.75, 0.0, 1.0),  # #7E7A80 2 階バルコニーの腰壁
    "panel_brown":     ((0.323, 0.195, 0.138), 0.70, 0.0, 1.0),  # #9A7A68 1 階の茶色いパネル
    "deck_wood":       ((0.175, 0.141, 0.136), 0.85, 0.0, 1.0),  # #746967 屋上テラスのデッキ
    "metal_charcoal":  ((0.042, 0.051, 0.065), 0.40, 0.60, 1.0),  # #3A4048 目隠しルーバー・サッシ
    # 入口（kcd_lib.entrances）。Unity 側の同名マテリアル（内装と共用）に合わせる
    "wall_accent_navy": ((0.090, 0.140, 0.260), 0.60, 0.0, 1.0),  # #55698B
    "plastic_black":   ((0.055, 0.055, 0.058), 0.50, 0.0, 1.0),  # #424244
    "light_panel":     ((1.000, 0.985, 0.930), 0.40, 0.0, 1.0),  # #FFFDF7
    # 外構
    "stone_light":     ((0.624, 0.604, 0.552), 0.75, 0.0, 1.0),  # #CFCCC4
    "stone_dark":      ((0.392, 0.371, 0.328), 0.78, 0.0, 1.0),  # #A8A49B
    "asphalt":         ((0.144, 0.144, 0.138), 0.92, 0.0, 1.0),  # #6A6A68
    "line_white":      ((0.900, 0.900, 0.880), 0.70, 0.0, 1.0),  # #F3F3F1
    ...
    "soil":            ((0.400, 0.300, 0.200), 0.94, 0.0, 1.0),  # #AA957C
...
# 背景建物用のクリーム〜グレー（bg_wall_0..5）。MaterialLibrary.CampusColors と同じ色
BG_COLORS = [
    ((0.571, 0.533, 0.451), 0.85),  # #C7C1B3
    ((0.445, 0.434, 0.397), 0.85),  # #B2B0A9
    ((0.366, 0.366, 0.356), 0.85),  # #A3A3A1
    ((0.638, 0.604, 0.539), 0.85),  # #D1CCC2
    ((0.275, 0.275, 0.279), 0.85),  # #8F8F90
    ((0.479, 0.445, 0.371), 0.85),  # #B8B2A4
]
