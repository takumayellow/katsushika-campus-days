...
内向きと扉の外向きの内積が -0.9（`DOOR_DOT_MAX`）以上、面に沿った差が 0.05 m
（`DOOR_LATERAL_TOL`）を越える、奥行きの差が 0 以下（原点が扉より内側）、`DOORS` に扉が無い、
のどれかで exit 1。奥行きの差（原点から扉まで）は、原点を面から外に取った距離に、扉のある外壁が
外接矩形の面から奥へ下がっている分を足したものなので、正であることだけを見る。
同じ棟に扉が複数あれば、入口を導くときと同じく `DOORS` の最初の行と照合する。
