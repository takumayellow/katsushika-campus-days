...
- `--no-export` 全棟: **約 6 秒**（うち近景の元の読み込み約 1 秒。`--no-ext` なら約 2 秒）
- 書き出し全棟（読み戻しの検証を含む）: **約 13 秒**
...
屋内の入口がキャンパスの扉と同じ面の同じ位置にあるかも棟ごとに測る
（`[verify] OK <id> 入口とキャンパスの扉: 向きの内積 -1.000 面に沿った差 0.000 m 奥行きの差 2.50 m`）。
内向きと扉の外向きの内積が -0.9 以上、または面に沿った差が 0.05 m（`DOOR_LATERAL_TOL`）を
越えると exit 1。奥行きの差（原点から扉まで）は、原点を面から外に取った距離に、扉のある外壁が
外接矩形の面から奥へ下がっている分を足したものなので判定に使わない。
...
- 同 `_summary.json` — 全棟の三角数・オブジェクト名・Empty 名・座席数・FBX サイズ。
  ...
  かかるので入れなかった本数 `skipped`）、`own`（入れなかった自分の棟の面の数。外装 `bld`、
  扉の部材 `door`）、`entrance`（入口の前の通り道にかかるので入れなかった設備の数 `props` と
  その面の数 `prop_faces`、木の本数 `trees`）
- `docs/previews/interior_<id>.png` ほか（`--preview` のとき。全 14 枚 1280×720）

## 棟ごとの内容と規模

合計 **296,065 三角形**（上限 300,000）。窓の外の近景は別に数えて **166,467 三角形**（上限 180,000、
1 棟 50,000）。どれかを越えると `[budget] NG` を出して exit 1。FBX の大きさは近景を含む。

| id | 建物 | 三角形 | 近景 | FBX | Empty | 中身 |
|---|---|---:|---:|---:|---:|---|
| `research1` | 第1研究棟 | 40,439 | 24,613 | 0.87 MB | 80 | 1F ロビー（受付・ソファ・掲示板・立席島）+ EV 3 基 + 廊下 142 m + 研究室 2 室 + 教授室 |
| `lecture` | 講義棟 | 47,429 | 43,851 | 1.35 MB | 162 | 3 層吹き抜けの大階段ホール + **大ホール 600 席**（16/18/16 × 12 列）+ ホワイエ + 演習室 3 室 + 中教室 60 席 |
| `research2` | 第2研究棟 | 64,210 | 25,594 | 1.12 MB | 36 | 1〜2F 吹き抜けの**大食堂 1,400 席**（配膳カウンター 38 m・券売機 6 台・返却口・2F 回廊 240 席） |
| `kyoso` | 共創棟 | 25,544 | 26,221 | 0.78 MB | 87 | 1F カフェ（吹き抜け）+ コンビニ、2F ラウンジ（ロビーの階段から吹き抜けを通って上がる）。計 90 席 |
| `library` | 図書館 | 57,924 | 12,969 | 0.83 MB | 84 | 開架書架 14 連 + 閲覧長机 6 列 + 個人閲覧ブース 66 席 + 2F 回廊の吹き抜け（閲覧室から回廊へ階段）。計 299 席 |
| `gym` | 体育館 | 20,965 | 11,721 | 0.46 MB | 30 | アリーナ（28×15 m コート・ゴール 2 基・ステージ）+ 観覧席 540 席 + 屋根トラス + 用具庫 |
| `lab1` | 実験棟1 | 22,502 | 10,895 | 0.47 MB | 17 | 廊下（ロッカー・掲示板・自販機）+ 実験室 2 室 + 準備室 1 室（実験台・ドラフト・ボンベ・薬品庫・試薬棚・流し） |
| `lab2` | 実験棟2 | 12,980 | 6,498 | 0.33 MB | 16 | 短辺（東面）の入口から玄関ホール（掲示板・ベンチ・鉢植え）+ 廊下 + 実験室 1 室 + 準備室 1 室（同上） |
| `greenhouse` | 温室 | 4,072 | 4,105 | 0.19 MB | 5 | 切妻ガラス屋根（棟木は入口の妻面から奥へ）・栽培ベンチ 4 列・鉢植え・灌水パイプ |
...
入口の面と面に沿った位置は、キャンパスの扉と同じにする（`kcd_interior/spec.py` の `_entrance_uv` が
`kcd_lib/entrances.py` の `DOORS` から導く）。屋内の外周は footprint の外接矩形なので、開口は
その矩形の面に置き、原点は面から棟ごとの距離（2.5〜6.5 m）だけ外に取る。
...
- 外周の内側に入る部分は切り落とす。
- 自分の棟の外装（`bld_<id>`）と、`bld_entrances` のうち自分の扉の部材（風除室・ガラス扉・枠・
  庇・マット）は入れない。外装の窓割りは屋内と別に作っているので、残すと外壁・ルーバー・柱廊・
  庇が屋内の窓の真ん前をふさぐ。扉の前の石張り（天端が `APRON_TOP` = 0.125 m 以下の面）は
  地面の一部なので残す。
- 屋内の入口の前に通り道を空ける。開口の両脇へ 1 m（`LANE_SIDE`）、外周から外へ 8 m
  （`LANE_DEPTH` = キャンパスで風除室の先に木・ベンチ・照明柱を置かない長さ `KEEP_CLEAR`）の
  矩形にかかる設備（花壇・ベンチ・照明柱・看板・自販機・ゴミ箱）は物ごと入れず、幹が矩形から
  0.5 m（`LANE_TREE`）以内の木も入れない。自分の棟の立て看板（キャンパスで入口の脇に立てたもの）と、
  地面・水面・ほかの棟は矩形にかかっても残す。
...
`kyoso` 0.50 m / `lab1` 0.15 m / `greenhouse` 0.10 m（計 0.75 m）が残るが、
いずれも屋根・棟木の継ぎ目（床の高さ 3.2〜8.6 m）で、spawn から届く立てる面の最高点
（lab1 1.44 / greenhouse 0.96 / kyoso 5.25 m）より上にあり、プレイヤーは立てない。最高点は 0.1 m 格子の
BFS で、上りはジャンプ（1.10 m）以内なら隣へ移れるとし、カプセルの半径は見ない（実際より高く出る）。
